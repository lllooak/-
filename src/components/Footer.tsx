    ...
    export function Footer() {
      ...
      async function fetchFooterPages() {
        try {
          ...
          const { data, error: fetchError } = await supabase // Renamed error variable
            .from('page_content')
            .select('slug, title')
            .eq('is_published', true)
            .order('title');

          if (fetchError) throw fetchError; // Throw the specific fetch error
          ...
          setError('שגיאה בטעינת התוכן'); // Use the defined setError
        } finally {
          setLoading(false);
        }
      }
