import React, { useEffect, useState } from 'react';
    ...
    // import { sendCreatorOrderNotificationEmail } from '../utils/emailService'; // Removed this import
    import { RequestDetails } from '../../../components/RequestDetails';

    interface Request {
      id: string;
      fan_id: string;
      creator_id: string; // Added creator_id for completeness, though it's the current user
      fan_name?: string;
      creator_name?: string; // Added for consistency if needed elsewhere
      request_type: string;
      status: string;
      price: number;
      message?: string;
    ...
    export function RequestsPage() {
      ...
      useEffect(() => {
        ...
        const initializeSubscriptions = async () => {
          try {
            ...
            if (!user) {
              console.log('User not authenticated for RequestsPage');
              setLoading(false);
              return;
            }
            cleanup = await initializeRealtime(); // This fetches initial data and sets up subscriptions
          } catch (error) {
      ...
      }, [initializeRealtime]); // Depend on initializeRealtime from the store

      const filteredRequests = (requests || []).filter(request => {
        const matchesFilter = filter === 'all' || request.status === filter;
        const fanName = request.fan_name || '';
        const requestType = request.request_type || '';
        const matchesSearch = 
          fanName.toLowerCase().includes(searchQuery.toLowerCase()) ||
          requestType.toLowerCase().includes(searchQuery.toLowerCase());
      ...
      const handleStatusUpdate = async (requestId: string, newStatus: string) => {
        try {
          ...
          // The initializeRealtime from the store should handle updating the local state
          // by re-fetching or through its own realtime subscription updates.
          // If the store's initializeRealtime doesn't immediately reflect changes,
          // you might need a more direct way to update the store's requests array here.
          // For now, relying on the store's mechanism.
          if (initializeRealtime) { // Check if initializeRealtime is defined
             await initializeRealtime(); // Re-fetch/update data
          }
        } catch (error) {
        ...
        <div className="space-y-6">
          ...
          {selectedRequest && (
            <RequestDetails
              ...
              onStatusUpdate={async () => { // Ensure onStatusUpdate is async if initializeRealtime is async
