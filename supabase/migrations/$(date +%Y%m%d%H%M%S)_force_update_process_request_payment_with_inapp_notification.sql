-- Drop existing function if it exists to ensure the new version is applied
    DROP FUNCTION IF EXISTS public.process_request_payment(uuid, uuid, uuid, numeric);

    -- Create/Recreate function to process request payment and trigger email & in-app notifications
    ...
    BEGIN
      ...
      BEGIN
        ...
        -- Update request status (assuming it was created with a different initial status or this confirms it)
        ...
        INSERT INTO public.notifications (
          ...
          p_creator_id,
          'בקשת וידאו חדשה',
          'התקבלה בקשת וידאו חדשה מ-' || COALESCE(v_fan_name, 'מעריץ'), -- Add fan name to message
          'request',
          p_request_id,
          'request'
        );
