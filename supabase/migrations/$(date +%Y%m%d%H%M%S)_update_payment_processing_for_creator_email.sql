-- Drop existing function if it exists to ensure it's updated
    DROP FUNCTION IF EXISTS public.process_request_payment(uuid, uuid, uuid, numeric);

    -- Create updated function with proper earnings handling and Edge Function invocation
    ...
    BEGIN
      ...
      BEGIN
        ...
        -- The request status is set to 'pending' by the frontend when creating the request.
        -- If it needs to be updated here, ensure it's the correct logic.
        -- For now, we assume the request is already 'pending'.
          ...
          BEGIN
            SELECT net.http_post(
                ...
                body:=jsonb_build_object(
                    ...
                    'orderPrice', p_amount,
                    'requestType', v_request_type -- Can be redundant, ensure consistency
                )
            ) INTO v_edge_function_response;
            
            -- Optionally log edge function response or handle errors
