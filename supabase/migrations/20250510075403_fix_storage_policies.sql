-- Ensure RLS is enabled on storage.objects -- This should be enabled by default by Supabase.
    -- Attempting to enable it again can cause ownership errors if the migrating user isn't the owner.
    -- ALTER TABLE storage.objects ENABLE ROW LEVEL SECURITY;

    -- Drop potentially conflicting old policies first to ensure a clean slate
    DO $$ 
    DECLARE
      policy_name text;
    BEGIN
